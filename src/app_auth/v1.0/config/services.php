...
use Phalcon\Di,
    ...
    WPLib\Session\Adapter\Redis as SessionAdapter,
...
$di->set('logger', function() use($config) {
    ...
    if (in_array(ENVIRON, ['develop', 'test'])) {
...
if (!IN_CLI) {
    ...
    $di->set('session', function() use($config) {
        if (session_status() != PHP_SESSION_NONE) {
            throw new Exception('Session状态异常');
        }

        // $server_num = crc32(session_id()) % count($config->session->servers);
        $server = $config->session->servers[0];
        $options = [
            'host'       => $server->host,
            'port'       => $server->port ?: '6379',
            'auth'       => $server->auth ?: $config->session->auth,
            'persistent' => false,
            'lifetime'   => 3600 * 6,
            'prefix'     => $config->session->prefix,
            'statsKey'   => $config->session->statsKey,
        ];
...
$di->setShared('cache', function() use($config) {
    $frontendcache = new FrontendCache($config['cache']['frontend']);

    $cache = new Memcached($frontendcache, $config['cache']['backend']);
 ...
 * 设置数据库服务
 */
$di->set('db_qy', function() use ($config, $di) {
    // 事件管理器
    $eventsManager = $di->getShared('eventsManager');
    ...
    //监听所有的db事件
    $eventsManager->attach('db_qy', $dbListener);

    $connection = new DbAdapter(array(
        'host' => $config->database_qy->host,
        'username' => $config->database_qy->username,
        'password' => $config->database_qy->password,
        'dbname' => $config->database_qy->dbname,
        'charset' => $config->database_qy->charset,
    ));
